...
public static class Quiz
{
    ...
    {
        return Enumerable.Range(1, (exclusiveUpperLimit >= 1) ? exclusiveUpperLimit - 1 : throw new ArgumentOutOfRangeException($"{exclusiveUpperLimit} is lower than 1")).Where(x => int.IsEvenInteger(x)).ToArray();
    }
    ...
    {
        return Enumerable.Range(1, (exclusiveUpperLimit >= 1) ? exclusiveUpperLimit - 1 : 1).Where(x => x % 7 == 0).Select(x => Convert.ToInt32(Math.Pow(x, 2))).OrderByDescending(x => x).ToArray();
    }
    /// <summary>
    ...
    {
        return text.Where(char.IsLetter).Select(char.ToUpper).GroupBy(c => c).Select(group => (group.Key, group.Count())).ToArray();
    }
}